...
import { createHarnessAdapter } from "../../packages/adapters/codebuddy/src/plugin.js";
...
for (const managedRemoteHost of [false, true])
  for (const hasSession of [false, true]) {
    test(`codebuddy command button opens its real adapter catalog (hasSession=${hasSession}, managedRemoteHost=${managedRemoteHost})`, async ({
      page,
    }) => {
      ...
      try {
        ...
        const cost = menu.locator('[data-command-id="codebuddy.cost"]');
        if (hasSession) {
          await cost.click();
