...
import { createHarnessAdapter } from "../../packages/adapters/hermes/src/plugin.js";
...
for (const managedRemoteHost of [false, true])
  for (const hasSession of [false, true]) {
    test(`hermes command button opens its real adapter catalog (hasSession=${hasSession}, managedRemoteHost=${managedRemoteHost})`, async ({
      page,
    }) => {
      ...
      try {
        ...
        const cost = menu.locator('[data-command-id="hermes.help"]');
        if (hasSession) {
          await cost.click();
