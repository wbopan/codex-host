...
The plugin uses the shared executable-discovery mechanism, including npm shims on Windows and version-manager roots on macOS/Linux. `CODEXHOST_CODEBUDDY_COMMAND` can select a specific installation. An explicit invalid installation does not silently fall back. Authentication, native settings, tools and MCP configuration remain CodeBuddy's responsibility. Headless Host never auto-accepts directory trust: Session open/inspect fails only when an existing `~/.codebuddy/settings.json` explicitly leaves the working directory out of `trustedDirectories` and `trustAll` is unset. Missing `settings.json` or an otherwise unknown trust status still launches ACP; if Model setup then fails, the error includes trust remediation hints. Per-Thread environment values override the factory's environment on both create and resume.
