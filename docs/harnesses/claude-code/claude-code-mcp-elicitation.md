...
Forms requiring user fields, URL authentication, unknown schema constraints, and messages longer than the UI's 500-character description limit are cancelled. A client limitation is not a user denial: native providers may persist a `decline`, so only an explicit denial from the approval UI returns it. No fields are invented, no URLs are automatically opened, and permission mode does not automatically answer elicitation. Tool approval remains separate from MCP server confirmation.

The Adapter sets `CODEXHOST_MCP_ELICITATION=1` in its Claude child environment to advertise this callback. The companion Desktop bridge can cancel unhandled requests on older Host versions instead of forwarding them to the SDK's default `decline` response.
